...
func init() {
	...
	catalog(ja, "Folder", "フォルダ")
	catalog(ja, "Load more...", "さらに読み込む...")
	catalog(ja, "Show the next page of results", "次のページを表示")
	catalog(ja, "S3 Object Information", "S3 オブジェクト情報")
	catalog(ja, "Delete %s? [y/N]", "%s を削除しますか？ [y/N]")
	catalog(ja, "Delete all objects under %s? [y/N]", "%s 以下の全オブジェクトを削除しますか？ [y/N]")
	catalog(ja, "Delete all objects under %s? (counting...) [y/N]", "%s 以下の全オブジェクトを削除しますか？（件数を確認中...） [y/N]")
	catalog(ja, "Delete %d objects under %s? [y/N]", "%[2]s 以下の %[1]d 件のオブジェクトを削除しますか？ [y/N]")
	catalog(ja, "Deleted %d objects under %s", "%[2]s 以下の %[1]d 件を削除しました")
	...
	catalog(ja, "Storage Class:", "ストレージクラス:")
	catalog(ja, "Content Type:", "Content-Type:")
	catalog(ja, "Version ID:", "バージョンID:")
	catalog(ja, "Metadata:", "メタデータ:")
	catalog(ja, "Prefix:", "プレフィックス:")
	catalog(ja, "Copied to clipboard", "クリップボードにコピーしました")
	...
	catalog(ja, "Select item / open folder", "項目を選択 / フォルダを開く")
	catalog(ja, "Descend into folder (also in delete mode)", "フォルダに入る（削除モードでも有効）")
	catalog(ja, "Filter items", "項目をフィルタ")
	catalog(ja, "Toggle object metadata preview", "オブジェクトのメタデータプレビューを切替")
	catalog(ja, "Generate presigned URL", "署名付きURLを発行")
